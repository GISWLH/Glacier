/*
Chunked single-year export for heavy regions.

Use this when a full region is too heavy even for one year.

Workflow:
1. Upload the slim region shapefile package as a table asset.
2. Set regionKey, exportYear, and chunk parameters.
3. Run one chunk at a time.
4. Download all chunk CSV files and merge locally.
*/
...
var exportPrefix = profile.exportPrefix + '_' + exportYear + '_chunk_' + chunkStart + '_' + (chunkStart + chunkSize - 1);
...
var allLakes = ee.FeatureCollection(lakeAsset).sort('lake_id');
var chunkLakes = ee.FeatureCollection(allLakes.toList(chunkSize, chunkStart));
...
var rows = chunkLakes.map(function(lake) {
