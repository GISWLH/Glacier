/*
Refined annual maximum open-water extraction for glacier-fed lakes.

Usage:
1. Upload one regional GeoJSON from E:\Glacier\data\prepared\core_region_batches as a table asset.
2. Set `regionKey`, `lakeAsset`, and `exportPrefix`.
3. Run one region at a time, starting with:
   - greenland_periphery
   - central_asia
4. Export the CSV and review QC fields before scaling to all regions.
*/
...
var exportPrefix = profile.exportPrefix;
var yearStart = 2000;
var yearEnd = 2024;
...
function renameBandsAndTagSensor(img) {
  ...
  var renamed = ee.Image(ee.Algorithms.If(
    isL57,
    img.select(
      ['SR_B2', 'SR_B4', 'SR_B5', 'SR_B7'],
      ['green', 'nir', 'swir1', 'swir2']
    ),
    img.select(
      ['SR_B3', 'SR_B5', 'SR_B6', 'SR_B7'],
      ['green', 'nir', 'swir1', 'swir2']
    )
  ...
  var stats = ee.Dictionary(ee.Algorithms.If(imageCount.gt(0), (function() {
    ...
    var waterAreaCol = col.map(function(img) {
      ...
      return ee.Feature(null, {
        area_km2: safeFirstNumber(areaStats, ['area_km2', 'area', 'sum'])
      });
    });
...
var allRows = ee.FeatureCollection(
  years.map(function(y) {
    ...
    return lakes.map(function(lake) {
      return summarizeLakeYear(lake, y, months);
    });
  })
).flatten();
